...
- (void)viewDidLoad
{
    ...
    /* Update the profile with an image from the current photo */
    PFFile *pictureFile = self.photo[kCCPhotoPictureKey];
    [pictureFile getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
        self.profilePictureImageView.image = [UIImage imageWithData:data];
    }];
    
    /* Access the current user using the key kCCPhotoUserKey on the photo property. Update the labels with this information. */
